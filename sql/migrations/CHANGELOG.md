...
## 2026-08-07 — 0073_final_verification.sql 로컬 재생 영구 제외

새 노트북에 로컬 Docker/Supabase 스택을 처음 세팅하고 `tools/apply_migrations.py`로 0000번부터 순차 재생하던 중 `0073_final_verification.sql`(자체 검증 스크립트)에서 재생이 멈춘 건에 대한 결정.

0073은 자체 검증 스크립트로, CRITICAL 검사 중 `fn catchmenu_kds.authorize_kds_release` 존재 여부를 확인한다. 이 함수는 `0157_authorize_kds_release_overload_and_redesign`(승인된 워크패킷 601020, ChangeContract 601024 Slice 1-3)에서 정당하게 `DROP FUNCTION`되었고(0157 line 357/361), 그 역할은 `catchmenu_payment.release_kds_after_payment()`로 이전되었다(0157 line 38/123 확인). 0158-0167 어디에도 동명 함수가 재생성되지 않으므로, 이 검사는 현재 아키텍처 기준으로 원리적으로 통과 불가능하다(순서 조정으로도 해결 불가 — Cursor 조사 + 로컬 DB 확인으로 검증됨: 재실행 시 0073은 142 PASS / 1 FAIL, 유일한 FAIL이 이 항목).

**결정**: 0073은 로컬 마이그레이션 재생에서 영구 제외한다. 실제 보관 위치는 `sql/_excluded_from_local_replay/0073_final_verification.sql`(즉 `sql/migrations/` 밖). 파일 내용은 수정하지 않았다(검증 로직 최신화는 별도 판단이 필요한 작업으로 이번 범위 밖).

migration_history 상태(정확한 실제 상태): 0073은 이번 재생 이전의 실패 시도로 인해 `success=false` 행이 1건 남아 있다. 따라서 성공 집계(`WHERE success=true` = 166)에는 포함되지 않으며, 이후 재생에서는 폴더 밖에 있어 다시 시도되지 않는다. (참고: 이 1건의 false 행을 삭제하면 문자 그대로 '시도 안 함/미등록' 상태가 되지만, migration_history 직접 변경은 이번 지시의 신중 대상이라 임의로 삭제하지 않았다 — Human 결정 대기.)

추가 (별개 사안, 이미 해소됨): 0073의 애초 실패 2건(`cron_job_executions` 테이블, `run_daily_close_batch` 함수 부재)은 authorize_kds_release 건과 무관한 forward-reference 문제였다. 두 객체는 `0144_create_cron_job_executions_and_daily_close`에서 생성되며, 이번 로컬 재생에서 0144가 정상 적용되어 해당 2건은 더 이상 blocker가 아니다(재실행 로그에서 `run_daily_close_batch` PASS 확인). 이 forward-reference 부류의 순서 문제 처리를 위해, CHANGELOG L147-153에 문서화된 21개(문서상 "19개") 파일 목록 중 로컬에서 실제로 걸린 파일들(0093 및 0100/0107/0108/0110/0113/0114/0118/0119/0121/0122/0123/0126/0127/0131/0132/0133/0134/0135)을 임시로 `sql/migrations/` 밖으로 옮겨 나머지를 먼저 재생(제약 확장 0140/0145/0146/0147 및 0144 포함)한 뒤, 되돌려 재실행하여 **실제로 재실행되어 통과**시켰다(클라우드에서 쓴 migration_history 수동 accept 방식은 사용하지 않음). 그 결과 0073을 제외한 166/167이 정상 적용됨.

향후 권고: 0073 같은 '특정 시점 스냅샷 검증' 스크립트는 프로젝트가 진화하면서 자연히 낡아질 수 있다. 000701 §47에서 확정한 6단계 나선 방법론의 5단계(SQL 구현+이중검증)에 이런 검증 스크립트를 나선 종료마다 갱신하는 관행을 반영할지 검토 권고(이번엔 코드 수정 범위 밖이므로 제안만 기록).

참고: catchmenu_meta.migration_history에는 0073의 실제 시도 실패 기록(success=false)이 1건 남아있다(성공 카운트 166에는 미포함). 이 기록은 의도적으로 삭제하지 않았다 - '실제로 시도했고 실패했다'는 사실을 보존하는 게 더 정직한 이력이라고 판단.

-- 0137_patch_missing_functions.sql
-- Purpose: Patch missing functions from 0133.
--          run_opening_checklist
--          run_integration_test
-- Note: Nested PROCEDURE not supported in Supabase.
--       Using array accumulation pattern instead.
...
BEGIN
  ...
  -- helper: append check result
  -- usage: set v_item/v_status/v_detail/v_action then call block
  ...
  v_status := CASE WHEN v_menu_count >= 5 THEN 'PASS' WHEN v_menu_count > 0 THEN 'WARN' ELSE 'FAIL' END;
  ...
  WHERE store_id = p_store_id
    ...
    AND menu_status = 'AVAILABLE'
    AND CASE
      WHEN allergen_info IS NULL THEN true
      WHEN jsonb_typeof(allergen_info) <> 'array' THEN true
      ELSE jsonb_array_length(allergen_info) = 0
    END;

  v_item   := 'Allergen registration (Food Safety Law)';
  v_status := CASE v_allergen_missing WHEN 0 THEN 'PASS' ELSE 'FAIL' END;
  v_detail := CASE v_allergen_missing WHEN 0 THEN 'All menus have allergen info'
               ELSE v_allergen_missing::text || ' menus missing allergen' END;
  v_action := CASE v_allergen_missing WHEN 0 THEN null
               ELSE 'Food Safety Law violation. Register allergen_info.' END;
  v_checks := v_checks || jsonb_build_object('item',v_item,'status',v_status,'detail',v_detail,'action',v_action);
  v_pass_count := v_pass_count + CASE v_status WHEN 'PASS' THEN 1 ELSE 0 END;
  v_fail_count := v_fail_count + CASE v_status WHEN 'FAIL' THEN 1 ELSE 0 END;
  v_warn_count := v_warn_count + CASE v_status WHEN 'WARN' THEN 1 ELSE 0 END;
  ...
  v_action := CASE v_staff_count WHEN 0 THEN 'Run upsert_staff()' ELSE null END;
  v_checks := v_checks || jsonb_build_object('item',v_item,'status',v_status,'detail',v_detail,'action',v_action);
  v_pass_count := v_pass_count + CASE v_status WHEN 'PASS' THEN 1 ELSE 0 END;
  v_fail_count := v_fail_count + CASE v_status WHEN 'FAIL' THEN 1 ELSE 0 END;
  v_warn_count := v_warn_count + CASE v_status WHEN 'WARN' THEN 1 ELSE 0 END;
  ...
  v_detail := CASE v_staff_no_pin WHEN 0 THEN 'All staff have PIN'
               ELSE v_staff_no_pin::text || ' staff missing PIN' END;
  v_action := CASE v_staff_no_pin WHEN 0 THEN null ELSE 'Run upsert_staff(p_pin_hash=...)' END;
  v_checks := v_checks || jsonb_build_object('item',v_item,'status',v_status,'detail',v_detail,'action',v_action);
  v_pass_count := v_pass_count + CASE v_status WHEN 'PASS' THEN 1 ELSE 0 END;
  v_fail_count := v_fail_count + CASE v_status WHEN 'FAIL' THEN 1 ELSE 0 END;
  v_warn_count := v_warn_count + CASE v_status WHEN 'WARN' THEN 1 ELSE 0 END;
  ...
  v_status := CASE WHEN v_hours_set = 7 THEN 'PASS' WHEN v_hours_set > 0 THEN 'WARN' ELSE 'FAIL' END;
  ...
  SELECT EXISTS (
    ...
    WHERE is_current = true
      AND (validation_result->>'overall_status') = 'VALID'
  ) INTO v_schema_valid;

  v_item   := 'DB schema validation';
  v_status := CASE v_schema_valid WHEN true THEN 'PASS' ELSE 'WARN' END;
  v_detail := CASE v_schema_valid WHEN true THEN 'Schema valid' ELSE 'Schema not validated' END;
  v_action := CASE v_schema_valid WHEN true THEN null ELSE 'Run run_final_validation()' END;
  v_checks := v_checks || jsonb_build_object('item',v_item,'status',v_status,'detail',v_detail,'action',v_action);
  v_pass_count := v_pass_count + CASE v_status WHEN 'PASS' THEN 1 ELSE 0 END;
  v_fail_count := v_fail_count + CASE v_status WHEN 'FAIL' THEN 1 ELSE 0 END;
  v_warn_count := v_warn_count + CASE v_status WHEN 'WARN' THEN 1 ELSE 0 END;
...
BEGIN
  ...
  IF p_scenario IN ('ALL', 'PATENT2') THEN
    ...
    ELSE
      v_test_name := 'Patent 2: KDS HOLD verify';
      v_passed    := true;
      v_detail    := 'No available menu (skipped)';
      v_results   := v_results || jsonb_build_object('test',v_test_name,'status','PASS','detail',v_detail);
      v_pass := v_pass + 1;
    END IF;
  END IF;
  ...
  v_test_name := 'pg_cron active jobs';
  v_passed    := v_cron_count >= 20;
  v_detail    := v_cron_count::text || ' active jobs';
  v_results   := v_results || jsonb_build_object('test',v_test_name,'status',CASE v_passed WHEN true THEN 'PASS' ELSE 'FAIL' END,'detail',v_detail);
  v_pass := v_pass + CASE v_passed WHEN true THEN 1 ELSE 0 END;
  v_fail := v_fail + CASE v_passed WHEN false THEN 1 ELSE 0 END;

...
> ⚠️ **`HD-0-A-2-10` 으로 Billing Review 가 `0-A-2` 범위에서 이월됐다.**
>
> **이 선언은 무효가 되지 않는다. 실행 책임이 이월된 것이다.**
>
> ```text
> 0-A-2           격리 사건 원장에 귀책 관련 컬럼을 두지 않는다
>                 자동 분류 · review 생성 · 청구 전 경고를 구현하지 않는다
>                 감사 기록은 유지한다 — 601809 A-7
>
> 후속 워크패킷    Billing Review 설계 시 이 선언을 이행한다

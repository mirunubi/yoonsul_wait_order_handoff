...
| **MEDIUM** | **F-4** `TP-M-08` clean baseline replay 가 `[93] 0093` `chk_error_domain` 에서 중단한다. canonical 23도메인 / fresh 18도메인, `MENU` 부재. `0170` 76파일 전이며 **이 워크패킷과 무관한 저장소 자체 결함** | **별건 워크패킷** — `0093` replay blocker |

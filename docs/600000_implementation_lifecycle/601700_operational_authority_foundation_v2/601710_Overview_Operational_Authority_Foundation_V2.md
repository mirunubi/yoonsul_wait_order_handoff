...
| External Provider Mapping (거래 provider / 실행 provider) | `601702` §1.43 — 경계는 확정했으나 물리 구조는 integration contract 확보 후. §3.1 참조 |
...
외부 provider 의 실제 identifier 구조가 확보되지 않았다.
특정 업체를 확정한 상태도 아니다.
...
계약·API 문서를 확보하기 전에는 물리 설계를 시작하지 않는다.
아래는 확보 대상 목록이며, provider 유형별로 구분한다.

**거래 provider — 주문·결제·정산**

| # | 확인 항목 | 왜 필요한가 |
|---|---|---|
| T-1 | 옵션이 구조화 필드인가 메모 문자열인가 | 구조화가 아니면 재고 차감·원가 계산으로 이어지지 않는다 |
| T-2 | 옵션 수량 표현이 가능한가 | "재료 +20g" 같은 정량 개인화의 전제 |
| T-3 | 외부 시스템의 주문 생성에 승인·권한 조건이 있는가 | 조건이 있으면 자체 채널 주문 경로가 막힌다 |
| T-4 | 주문·상태 이벤트를 수신할 수 있는가 | 수신 못 하면 우리 쪽 상태 추적이 끊긴다 |
| T-5 | 중복 주문 방지용 외부 주문 키를 지정할 수 있는가 | 네트워크 재시도 시 주문이 두 번 들어가는 것을 막는다 |

**실행 provider — 주방·안내 화면**

| # | 확인 항목 | 왜 필요한가 |
|---|---|---|
| E-1 | 자체 주문 시스템에서 주문을 넣는 경로가 무엇인가 | REST / local daemon / socket / file 중 무엇이냐에 따라 adapter 구조가 달라진다 |
| E-2 | 구조화 modifier 를 손실 없이 수신하는가 | 손실되면 주방에서 개인화 주문을 읽을 수 없다 |
| E-3 | E-2 가 불가하면 메모 필드에 몇 글자까지 표시되는가 | fallback 표시 문자열의 길이 제약을 정한다 |
| E-4 | 취소·변경을 같은 경로로 보낼 수 있는가 | 주문 정정이 주방에 반영되지 않으면 현장에서 사고가 난다 |
| E-5 | 조리 시작·완료 상태를 회신하는가 | 회신이 없으면 재고 확정과 고객 안내가 불가능하다 |
| E-6 | 거래 provider 와의 연동 실적이 있는가 | 실적이 있으면 거래 provider 를 경유하는 경로를 검토할 수 있다 |

**채널 통합 범위**

| # | 확인 항목 | 왜 필요한가 |
|---|---|---|
| C-1 | 실행 provider 가 외부 주문 채널(배달 등)을 직접 수신하는가 | 수신한다면 우리가 각 채널을 개별 연동할 필요가 줄어든다 |
| C-2 | C-1 인 경우 그 주문을 우리 쪽으로 전달할 수 있는가 | 전달받지 못하면 해당 판매가 우리 재고·분석에서 누락된다 |

> ⚠️ **외부 채널 주문을 우리 시스템이 받지 않는 것은 Deferred 이지 불필요가 아니다.**
>
> ```text
> 재고 차감    외부 채널 판매를 모르면 실제 재고와 어긋난다
> 통합 매출    채널별 합산이 불완전해진다
> 메뉴 분석    채널별 판매 구성을 볼 수 없다
> 자동 발주    모든 판매 → 재료 소비 → 재고 → 발주 경로가 끊긴다
> ```
>
> 초기 운영에서 외부 채널을 실행 provider 쪽에 맡기더라도,
> **위 네 기능이 필요해지는 시점에는 반드시 우리 쪽으로 들어와야 한다.**

**확인 방식**

- 문서로 답이 나오지 않는 항목은 **실제 화면 데모**를 요청한다.
  특히 E-2·E-3 은 문서보다 화면이 정확하다.
- 확인 결과는 **provider 유형별로 기록**한다. 특정 벤더를 전제하지 않는다.

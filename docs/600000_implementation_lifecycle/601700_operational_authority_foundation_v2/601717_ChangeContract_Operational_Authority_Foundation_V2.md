...
| `601717_ChangeContract_…V2.md` | 15판 | git commit | **Stage 7 승인 시 §10.9 에 실측 기록** |
...
>          커밋 해시를 적어도 그 기록이 새 커밋을 만든다
> ```
>
> **`601717` 의 고정값은 이 표에 미리 적을 수 없다.**
>
> 이전 판은 「`50f96e59…` 이후 최초 커밋」이라는 상대 표현을 썼으나,
> 그 뒤로 `601717` 이 여러 번 갱신되어 **가리키는 대상이 어긋났다.**
>
> **Stage 7 승인자가 승인 시점에 실측해 §10.9 에 기록한다.**
>
> ```text
> git log -1 --format="%H" -- <601717 경로>
> ```
>
> 그 값이 확정 고정값이며, 구현자는 §10.9 의 값과 대조한다.
...
601717 기준 커밋 해시     git log -1 --format="%H" -- <601717 경로>
                          — 이 값이 §10.8 의 확정 고정값이다
601716 SHA-256 재측정값   §10.8 기록값과 일치 확인
                          — 측정 전 대조 절차 0단계(EOL 확인)를 수행한다
```

> ⚠️ **기록 시점 — 순서를 지킨다**
>
> ```text
> 1  601717 을 더 이상 수정하지 않는 상태로 만든다
> 2  Stage 7 셀을 승인 상태로 바꾼다
> 3  그 변경을 커밋한다
> 4  커밋 해시를 측정해 이 절에 기록한다
> 5  기록 자체를 다시 커밋한다
> ```
>
> **4단계에서 측정한 해시가 「승인된 계약」의 고정값이다.**
> 5단계 커밋은 기록 행위이며 계약 내용을 바꾸지 않는다.
>
> **구현자는 4단계 해시와 대조한다.**
> 현재 최종 커밋이 5단계 커밋이면 그 차이는 이 기록 추가분뿐이다.
>
> ⚠️ **5단계 이후 `601717` 이 다시 바뀌면 승인 효력이 그 변경분에 미치지 않는다.**
> 변경이 필요하면 Stage 7 을 다시 수행한다.
...
승인 대상 판본
  ...
  601717 기준 커밋       01cfd45bab7710b0db2a4957b7e540bebfad7377
```

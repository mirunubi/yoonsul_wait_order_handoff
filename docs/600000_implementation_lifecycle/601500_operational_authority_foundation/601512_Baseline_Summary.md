...
>
> ## 이 문서를 복구 기준선으로 사용하지 마시오
>
> 아래 본문은 0-A 당시의 설계·구현 기준선을 보존한 **HISTORICAL BASELINE**이다.
> Stage 7(Human Approval) 미수행, 1단계 Human 업무규칙 선언 부재,
> 원천 설계문서(`000150`/`000170`) 미대조 상태에서 완료 판정된 사실이 확인되어
> **현재 설계·구현의 권위 자료로 사용할 수 없다.**
>
> ### §2 "확정된 것 (재논의 금지)" — 효력 정지
>
> §2의 **"재논의 금지" 지위를 해제한다.**
> §2에 적힌 내용(`companies` 폐기, `legal_entities` 중심 모델, 상태 2컬럼 분리 등)은
> **확정 사항이 아니라 당시의 설계 판단**이다. 새 0-A에서 전부 재검토 대상이다.
>
> ### §1 "다음 할 일" — 전부 HOLD
>
> 0-A-2 착수, 0-A-3, 0-B — **전부 보류.** 0-A 권위 보류로 선행조건이 해소되지 않았다.
>
> ### 완료일 정정
>
> 본문의 `2026-08-11` 완료 기록은 **오기**다.
> 실제 커밋 시각은 `2026-08-10 16:14:09 KST`.
> (오기 사실 자체를 기록으로 남기기 위해 본문 날짜는 이 배너에서만 정정한다.)
>
> ### 이 문서를 여는 사람에게
>
> 이 문서는 **무엇이 결정됐는가**가 아니라
> **무엇이 시도되었고 왜 무효가 되었는가**를 알기 위해 읽는다.
> 새 0-A는 원천 설계문서 검증부터 시작한다 — 이 문서에서 시작하지 않는다.
>
> **판정: 정영석 / 2026-08-10**
...
| 항목 | 상태 |
|---|---|
| **워크패킷 상태** | ✅ **0-A 완료** — **Stage 12 Human Merge 승인 (2026-08-11)** |
| **적용 마이그레이션** | `0168`(신규 테이블 4 + 컬럼 3) · `0169`(전용 owner role + SOLE 유일성) — **둘 다 로컬 적용 완료** |
| 설계 문서 | `601501`(ERD **v5**) / `601502`(Overview v5) / `601503`(Logic v5) — 3단계 대조 2회 + Stage 11B 반영 |
| 계약·검증 | `601505`(ChangeContract **v3**) / `601504`(TestPlan) — 전 항목 PASS |
| 감사 | `601509`(11A) → **`601510`(11B `BLOCK`)** → **`601511`(11A 재감사 `APPROVE_WITH_NOTES`, Stage 11 종결)** |

### 완료 시점에도 남아 있는 것

| 항목 | 이월처 |
|---|---|
| **Stage 11B 조건 ②**(`SECURITY DEFINER` 보안규칙) | **0-C — §5A 필독** |
| Open Item (m) 클라우드 미검증 | 클라우드 배포 시 |
| `ACTIVE`+`ISOLATED` 동시상태의 **과금정책 미정** | **0-A-2 착수 전 결정**(`601511`) |
| "행위기준 완료조건"의 **CI 기계적 강제 부재** | 프로젝트 구조적 공백(`601511`) |

> **⚠️ 완료 ≠ 안전**: 0-A는 `isolate_tenant()`를 **의도적 장애 상태**로 남긴 채 병합됐다.
> `601505` §4의 금지 조항(호출 금지 · `ACTIVE` 승격 금지 · 신규 호출자 배포 금지)은
> **0-A-2 완료까지 계속 유효**하다. §3을 반드시 읽을 것.

### 다음 할 일

1. ⭐ **0-A-2 착수** — `601505` §8A에 따라 **0-B보다 우선**. 착수 전 `ACTIVE`+`ISOLATED` 과금정책 결정 필요
2. 0-A-3 (`onboard_tenant`/`provision_tenant` 재설계)
3. 0-B(Staff identity/session)는 **착수 가능** 상태이나 §8A 순서상 0-A-2 이후 권장

...
- `600481_Overview.md` (incl. §0.5 Human final decision, 2026-07-14)
- `600482_Logic.md`
- `600483_TestPlan.md`
...
- `p_locale` and a hypothetical `p_options jsonb` extension field are **not** introduced (Human decision, YAGNI — `600481_Overview.md` §0.5).
...
|---|---|
| `sql/migrations/0027_create_payment_intent_rpc.sql` | Original 8-param definition — already confirmed correct (`600482_Logic.md` §4). No edit needed or approved. |
| `sql/migrations/0038_create_toss_webhook_processor_rpc.sql` | Real caller 1 — already uses the exact 8 named arguments matching the canonical signature (`600481_Overview.md` §1.1). No edit needed. |
...
- Patent 1 design invariant: `kds_release_authorized` stays `false` from this function; `authorize_kds_release()` remains a fully separate, later step (confirmed not called from within `confirm_payment_from_provider()`, `600481_Overview.md` §1.3).
...
Implementation must be verified against `600483_TestPlan.md`. Required verification groups:
...
Both confirmed to have zero live callers (`600481_Overview.md` §2). Same overload-sprawl root cause as `confirm_payment_from_provider()`, but out of scope here. When addressed:
...
Discovered during `600490_customer_handoff_contract_reconciliation`'s Contract Inventory (Track 1): the same "later migration adds a new overload instead of replacing the original" pattern also exists for:
...
- This is the first time the 8-param function will actually execute end-to-end in this project (Test C/D) — there is a small chance an undiscovered defect surfaces only under real execution, despite thorough static verification (`600482_Logic.md` §4).
...
- `0027`'s correctness was independently re-verified against live schema/constraints twice already (`600480`'s investigation, `600482_Logic.md` §4) before this contract was written.

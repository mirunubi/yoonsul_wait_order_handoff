...
| `sql/migrations/0081_create_customer_app_rpc.sql` | L1142 | `catchmenu_store.track_takeout_order(...)`, JSON response | `'request_memo', v_order.request_memo,` | `'memo', v_order.memo,` — output JSON key also changes (A안, Human decision 2026-07-11): repo-wide search found no consumer of this key (see `600912_Logic.md` §2.1 rationale); this reverses the earlier "keep the output key" decision for this specific function |
...
- Backward-compatible output JSON keys may remain `request_memo` only where `600912_Logic.md` explicitly says to preserve the response contract (currently only `0099`'s `get_kds_realtime_state()` L448 — see Open Item 8 in §7 for the resulting inconsistency).
...
5. Confirm the live definitions contain both:
   ...
   - current `600910` repairs (`memo`, `severity`, `UNDER_INVESTIGATION`)
...
These are copied from `600912_Logic.md` §5 and must not be solved inside this batch unless a later approval explicitly expands the boundary:

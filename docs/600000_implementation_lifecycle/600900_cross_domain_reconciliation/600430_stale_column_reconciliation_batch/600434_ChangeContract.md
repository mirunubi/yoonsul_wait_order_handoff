...
| `sql/migrations/0084_create_reconciliation_advanced_rpc.sql` | `case_severity` -> `severity`; `'INVESTIGATING'` -> `'UNDER_INVESTIGATION'`; only the confirmed occurrences in `600432_Logic.md` §2.2/§2.3. |
...
- `gap_amount` appears in `0111` / `0120`, but `600432_Logic.md` §5 classifies it as a separate follow-up issue.
- `layer_number`, `amount_difference`, and `case_description` appear in `0084`, but `600432_Logic.md` §5 classifies them as outside this batch.
...
Verification must follow `600433_TestPlan.md`.
...
## 7. Open Items Copied From `600432_Logic.md` §5

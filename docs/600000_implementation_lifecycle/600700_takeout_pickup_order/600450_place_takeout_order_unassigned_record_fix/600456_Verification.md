...
| Group 4 — Member + coupon provided | PASS (record-unassigned class) — no crash. Fails earlier, at the same `discount_pct` column error as Group 2 (confirmed consistent with `600452_Logic.md` §5's prediction). |
...
### 2.3 `discount_pct` (carried over from `600451_Overview.md` §3, re-confirmed and scope-corrected)
...
- **Scope correction**: `600451_Overview.md` §3 characterized this as blocking only "member+coupon." Independent testing this turn shows it blocks **any** call with `p_coupon_issue_id` provided, guest or member alike (Group 2 confirmed) — the coupon SELECT is gated by `p_coupon_issue_id is not null`, not by customer identity.

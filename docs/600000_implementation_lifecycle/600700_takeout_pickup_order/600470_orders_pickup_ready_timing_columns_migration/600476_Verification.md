...
| Design match vs. `600472_Logic.md` §1 | PASS — nullable, no default, no CHECK, matches `0013`'s existing timestamp-column pattern. |
...
Re-ran with a fresh `correlation_id` (`verify-600477-testA-rerun2`), `BEGIN`/`ROLLBACK`-wrapped:

- The `requested_pickup_at` failure at the `orders` INSERT (previously blocking, `600466_Verification.md`) is **gone** — execution now proceeds past it.
- **New failure**, immediately after, at the `order_items` INSERT:
  ```
  ERROR: column "unit_price" of relation "order_items" does not exist
  ```
- See `600477_Audit.md` for full root-cause analysis — this is broader than a single stale column name.

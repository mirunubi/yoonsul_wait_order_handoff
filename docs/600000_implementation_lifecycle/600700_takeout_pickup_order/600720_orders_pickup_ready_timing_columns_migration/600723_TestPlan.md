...
select catchmenu_store.place_takeout_order(
  ...
  p_correlation_id := 'verify-600723-place-takeout'
);

select id, order_number, requested_pickup_at, ready_at
from catchmenu_pos.orders
where correlation_id = 'verify-600723-place-takeout';
...
select catchmenu_store.call_customer_pickup(
  ...
  p_target_zone := null,
  p_locale := 'ko',
  p_correlation_id := 'verify-600723-call-pickup'
);

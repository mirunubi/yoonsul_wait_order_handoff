...
|---|---|---|---|
| `600500_payment_confirmation/` | 7 (`600480/`) | 2 | 9 |
| `600600_waiting_order_session/` | 14 (`600460/`=7, `600490/`=7) | 2 | 16 |
| `600700_takeout_pickup_order/` | 14 (`600450/`=7, `600470/`=7) | 2 | 16 |
| `600800_did_implementation/` | 4 (`600510/`=4, `600330/`=0 + `.gitkeep`) | 2 | 7 |
| `600900_cross_domain_reconciliation/` | 7 (`600430/`) | 2 | 9 |
...
Expected: exactly 3 workpacket subfolders (`600410_...`, `600420_...`, `600440_...`) plus the flat files `600400_Readme_KDS_Implementation.md`(renamed), `600401_ChangeHistory.md`, `600402_NavigationMap.md`(reduced to 3 rows), `600403_DecisionLog.md`, `600404_PlaceTakeoutOrder_Defect_Roadmap.md`, and this workpacket's own `600520_domain_folder_reorganization/`. `600330`, `600430`, `600450`, `600460`, `600470`, `600480`, `600490`, `600510` must **not** appear here anymore.
...
| `600502_NavigationMap_Payment_Confirmation.md` | 1 (`600480`) |
| `600602_NavigationMap_Waiting_Order_Session.md` | 2 (`600460`, `600490`) |
| `600702_NavigationMap_Takeout_Pickup_Order.md` | 2 (`600450`[newly authored, previously missing], `600470`) |
| `600802_NavigationMap_Did_Implementation.md` | **0** — `600510` deferred (Human decision, `600522_Logic.md` §1.1.1: still Stage 2, not yet Stage 6 ACCEPT, backfilled separately once it is); `600330` has no row, it's empty. File exists as an empty-shell skeleton. |
| `600902_NavigationMap_Cross_Domain_Reconciliation.md` | 1 (`600430`) |

Total rows across all 6 files: 3+1+2+2+0+1 = **9** (revised — `600522_Logic.md` §1.1.1 corrected this from an earlier "10" that mistakenly included a `600510` row that never existed in the original `600402` either), matching "8 original rows + 1 newly-authored `600450` row."
...
Purpose: the highest-value check in this TestPlan — confirm the 47-item backfill (`600522_Logic.md` §1.2, revised down from an earlier 51-item draft that mistakenly included `600510`) is complete and accurate, not just "looks plausible."

Execution:

```powershell
# Count actual files across 7 of the 8 moved-workpacket folders (600510 excluded, §1.1.1) + 600410
$paths = @(
  "600500_payment_confirmation/600480_confirm_payment_from_provider_overload_ambiguity",
  "600600_waiting_order_session/600460_takeout_session_type_fix",
  "600600_waiting_order_session/600490_customer_handoff_contract_reconciliation",
  "600700_takeout_pickup_order/600450_place_takeout_order_unassigned_record_fix",
  "600700_takeout_pickup_order/600470_orders_pickup_ready_timing_columns_migration",
  "600900_cross_domain_reconciliation/600430_stale_column_reconciliation_batch",
  "600400_kds_did_implementation/600410_kds_capacity_gate_and_status_reconciliation"
)
# 600800_did_implementation/600510_did_display_state_overload_and_legacy_defect is
...
**Negative sub-check (required)**: also `grep` both index files for `600511_Overview_Did_Display_State_Overload`, `600512_Logic_...`, `600513_TestPlan_...`, `600514_ChangeContract_...` — none of these 4 filenames must appear anywhere in `000005`/`000007` yet.

PASS condition: all 47 expected files (42 moved + 5 newly-backfilled `600410` entries) appear in both index files, zero missing, zero duplicated, path strings in `000005` match the actual post-move filesystem paths exactly (case-sensitive, correct domain folder name); **and** the negative sub-check confirms zero `600510`-related entries.
FAIL condition: any file present on disk but absent from either index (silent gap, the exact failure mode `600521_Overview.md` §3.2 already found once) — any index entry pointing to a path that doesn't exist on disk (stale/wrong path) — or any `600510` file appearing in either index (scope violation of §1.1.1's deferral).
...
(Post-move, the remaining 4 — `600484_ChangeContract.md`, `600491_Overview.md`, `600495_Module.md`, `600511_Overview_...md` — will have moved to their new domain folders; re-run the equivalent `git diff --stat` against their new paths.)
...
4. Test D — `000005`/`000007` 1:1 match against actual filesystem for all 47 backfilled items, zero gaps, zero stale paths, and zero `600510`-related entries (deferred per §1.1.1).

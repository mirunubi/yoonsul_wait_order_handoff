...
Derived from `600491_Overview.md`/`600492_Logic.md` (finalized) plus this turn's Correction-scope decision:
...
Fix design (exact Before/After for `600494_ChangeContract.md` §1):
...
insert into catchmenu_pos.orders (
  ...
  'W-TEST-600493', 'DINE_IN', 'CONFIRMED',
...
--------------------------------------+------------------+------------
 310973f4-bebb-4206-ab31-b5e66d23b84a | W-TEST-600493-01 | HOLD
(1 row)
INSERT 0 1
ROLLBACK
```
...
FAIL condition: the function either still fails on `max_waiting_count`/`max_wait_number` (fix ineffective) or succeeds completely (would mean the 8-column drift list in `600492_Logic.md` §1.1 is stale and needs re-verification).
...
3. Test C's blocker chain is recorded as an Open Item in `600494_ChangeContract.md`, not silently left implicit.

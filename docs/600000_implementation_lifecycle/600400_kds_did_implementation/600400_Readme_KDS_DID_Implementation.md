...
This folder owns controlled implementation lifecycle documentation for **KDS(주방 디스플레이)/DID(매장 디스플레이) 관련 결함 발견 및 정정 작업**을 다룬다. `000701_Guide_Controlled_AI_Development_Pipeline.md` §35(신설, 이 모듈의 작업 방식과 동시에 확정됨)의 "전수 문서 조사 폐기, 결함-기반 문서 연결" 원칙을 이 모듈에서 처음 적용한다.
...
- KDS/DID 런타임 결함 발견 및 정정 워크패킷(시작: `600410_kds_capacity_gate_and_status_reconciliation/`)
...
- 이 모듈 밖 결함(결제/고객식별/Flutter 등)을 이 모듈에서 다루는 것 — 각자의 모듈(`600100`/`600200`/`600300`)로
...
| --- | --- | --- |
| `600330_kds_did_event_reactive_implementation/` | (기존 존재, `.gitkeep`만 있음 — 이번 change와 무관, 손대지 않음) | 미상 |
| `600410_kds_capacity_gate_and_status_reconciliation/` | `kds_capacity_gate_and_status_reconciliation` | Draft / Stage 1.5 |

## File List

| Number | File | Status |
| --- | --- | --- |
| 600400 | `600400_Readme_KDS_DID_Implementation.md` | Draft |

...
## §4 `600410`/`600420`/`600430`에서 이미 처리된 부분과의 경계

- **`0098`/`0099`/`0106`/`0116`**: 이번 턴 직접 재확인 결과 이 4개 파일은 `kds_status`에 `'COMMITTED'`를 **이미** 사용하고 있으며(`READY_TO_COMMIT` 0건), `READY_TO_COMMIT` 문자열이 전혀 등장하지 않는다. `600420`/`600430` 배치가 이들을 이미 정상 상태로 만들어뒀다는 전제는 **이번 재확인으로 사실 확인됨** — 이 4개 파일은 이번 변경의 대상이 아니며 손대지 않는다.

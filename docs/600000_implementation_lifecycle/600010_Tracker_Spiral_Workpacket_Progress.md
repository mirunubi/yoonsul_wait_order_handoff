...
- **Stage 12 통과 후에도 사후 판정으로 권위가 보류될 수 있다.** 그 경우 `완료` 대신 **`권위보류`** 로 표기하고, 과거 완료 기록은 삭제하지 않고 병기한다(`600020` §1.1).
- **권위보류된 워크패킷을 재수행할 때는 새 워크패킷 번호로 별도 행을 만든다.** 기존 행은 역사적 기록으로 남기고 삭제하지 않는다.
...
| **0-B** Staff identity / session | 미배정 | — | ✅ **HOLD 해제 (2026-08-24)** — 0-A 재수행 Stage 12 완주로 선행조건 회복. **미착수** — 착수 전 선행 판정 필요: `catchmenu_common` 의 `auth_sessions` · `login_attempts` · `phone_verify_codes` · `security_tokens` 지위 판정(`000221` §4.3) | 2026-08-27 |
...
| **0-A-2** Tenant lifecycle / RPC · batch 정합 | `601800_tenant_lifecycle_rpc_alignment` | **Stage 6 Round 1 INCOMPLETE** | ⛔ **권위보류 (AUTHORITY SUSPENDED, 2026-08-31)** — 사유·근거: `600021` §1. `601801` 이 `010004` · `010640` 인용 0건 — 격리를 선언하면서 격리 정책 문서를 보지 않았다. 재수행 `601900`. 과거 기록: 착수(2026-08-27), Human Gate A RESOLVED(`601801` HG-A-1~HG-A-16), Stage 6 Round 1 blocking 15건 | 2026-08-31 |
| **0-A-2 재수행** Tenant isolation axis | `601900` (미개설) | **착수 전** | `600021` §2 구속 적용 — Stage 0 증거수집에 `010004` · `010640` · `000150` · `000170` · `000190` 포함. 4단계 착수 전 evidence pack 필수(`601708`·`601709` 형식). `601800` 을 답안지로 사용하지 않는다 | 2026-08-31 |

> ⛔ **2026-08-10 권위 재설정**: 본 트래커의 0-A 완료 기록은 **현재 권위가 없다.**
> Stage 7(Human Approval) 미수행, 1단계 Human 업무규칙 선언 부재,
> 원천 설계문서 `000150`/`000170` 미대조가 확인되었다.
> 판정 전문은 **`600020\_Governance\_Implementation\_Lifecycle\_Authority\_Reset.md`** 참조.
> 아래 §1.1~§2의 기록은 **당시 무엇이 기록되었는가**를 보존한 것이며,
> **지금 무엇을 해야 하는가의 근거가 아니다.**

### §1.1 0-A / 601500 완료 기록

> ⛔ **AUTHORITY SUSPENDED (2026-08-10)** — 아래는 **역사적 기록**이다.
> Stage 표에 **Stage 6·7이 없다.** `601505` §10은 두 단계 모두 `대기`로 기록하고 있으며,
> §9.3 승인란은 공란이다. 그 상태에서 Stage 8이 진행되었다.
> "전 Stage 통과"라는 서술은 사실과 다르다.
> **다음 필수 워크패킷: 0-A-2** 라는 아래 지시도 **HOLD**되었다(`600020` §1.2).
> → **2026-08-24 HOLD 해제**(0-A 재수행 Stage 12 완주), **2026-08-27 착수**(`601800`).
>
...
| `tenant` `ACTIVE`+`ISOLATED` 동시상태의 **과금정책 미정** | **0-A-2 착수 전 결정 필요**(`601511`) → **2026-08-27 `601801` HG-A-1~HG-A-9 로 해소** |
...
| **0-A-2** | Tenant lifecycle / RPC / batch alignment — candidate scope: `isolate_tenant`/`manage_subscription`/`tenant_status` 필터/`is_registered` | ⛔ **권위보류 (AUTHORITY SUSPENDED, 2026-08-31)** — 사유·근거 `600021` §1. `601801` 이 `010004` · `010640` 인용 0건. 재수행 `601900`. 과거: HOLD 해제(2026-08-24) 후 착수(2026-08-27) `601800`, Human Gate A RESOLVED, Stage 6 Round 1 INCOMPLETE. **`601505` §4 금지 조항은 0-A-2 완료까지 유효**(`000221` §4.1) |
| **0-A-2 재수행** | Tenant isolation axis — `601800` 권위보류에 따른 재시작 | **미착수 — `601900` 미개설.** `600021` §2 구속 적용 — Stage 0 증거수집에 `010004` · `010640` · `000150` · `000170` · `000190` 포함. 1단계 선언을 상위 정책 위에서 도출하고, 4단계 착수 전 evidence pack 을 만든다. `010004` §24 11항 선언 여부를 판정한다. `601800` 을 답안지로 사용하지 않는다(`600021` §2) |
| **0-A-3** | Provisioning Integrity — `onboard_tenant` / `provision_tenant` 재설계 | ✅ **HOLD 해제 (2026-08-24)** — 0-A 재수행 완주로 선행조건 회복. **미착수 — 0-A-2 다음**. 범위: C-3 + provisioning RPC 재설계 + tenant·merchant_account 생성 원자성 + `stores.merchant_account_id` `NOT NULL` 승격. **C-3 는 H-2 · H-3 · RLS policy · grant · RPC write path 보다 선행**(`601748` §8). 근거 `000221` §4.2 |
| **0-A 역전파** | 상위 정본문서(`000150`/`000170`/`003020`/`009030`/`010004`) 역전파 — 문서 전용, `.sql` 없음 | ⚠️ **적용 완료 — 적용분 재검토 대상**(`601600`, 커밋 17:26). 0-A 권위보류에 연동 |
| **0-B** | Identity / Auth / Session / Invitation — Staff identity / session | ✅ **HOLD 해제 (2026-08-24)** — 0-A 재수행 Stage 12 완주로 선행조건 회복. **미착수** — 착수 전 선행 판정 필요: `catchmenu_common` 의 `auth_sessions` · `login_attempts` · `phone_verify_codes` · `security_tokens` 지위 판정(`000221` §4.3) |

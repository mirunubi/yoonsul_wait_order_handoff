...
프로젝트 기반 정의. 프로젝트 정체성, BM 특허 경계, 핵심 개념, 외부 시스템 분리 원칙. 하위에 000300(문서 거버넌스), 000400(개발 기반) 서브도메인 포함.
...
### 001000_mvp_scope
MVP 범위 정의. 1차 출시 포함/제외 기능 경계, MVP 단계별 기능 목록, 우선순위. Stage 0 POS-less 진입 런타임(QR 요청, 001205) 상세 레인 포함.

### 003000_saas_runtime
SaaS 런타임 아키텍처. 멀티테넌트 구조, 테넌트 격리, SaaS 운영 모델, 런타임 경계, Entry Media(QR/NFC 진입 자산) 인벤토리 거버넌스.
...
고객 런타임 구현 준비. 매장 런타임 WorkPackage, 입장/대기 고객 런타임 경계, 증거 핸드오프, 고객 표시 제어 문서.

### 007000_admin_console
어드민 콘솔 스펙. 운영자/오너 관리 화면, 테넌트 관리, 대시보드 기능 정의.

### 008000_ai_customer_center
AI 고객센터. AI 상담 자동화, 고객 문의 처리, 지식베이스 연동. 고위험 매장 운영/주류 판매 헌법(성인 확인, 미성년자 접근 방지, 야간 안전, 분쟁/환불 증거)이 폴더 내용의 상당 부분을 차지.
...
앱/API 프로젝션. 고객·매장·관리자·지원 화면과 개념적 API 그룹, 권한/상태 투영 경계.

### 014000_pos_provider_integration_strategy
POS 공급업체 통합 전략. 공급업체 평가, 연동 전략, 어댑터 설계, 첫 매장 롤아웃 및 운영 프로그램.
...
구현 계획 (확장 밴드). 022000 이후 추가 구현 계획 문서. 022000 이후 확장 밴드 — AI 워크플로우/워크패키지 거버넌스 계층.

### 024000_deployment_operations
배포 운영. 배포 절차, 운영 지원, 롤백 계획. (운영/지원 준비 중심)
...
배포 운영 릴리즈 런타임 통제. 릴리즈 관리, 런타임 통제 정책. (릴리즈 실행 통제 중심)

### 028000_future_expansion
향후 확장. 멤버십/포인트/프랜차이즈 미래 모델과 장기 참조 경계.

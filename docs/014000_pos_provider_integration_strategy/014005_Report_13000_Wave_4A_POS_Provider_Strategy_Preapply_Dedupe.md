# 014005_Report_13000_Wave_4A_POS_Provider_Strategy_Preapply_Dedupe
...
**Source plan:** `docs/000006_Plan_Top_Level_Folder_Consolidation.md` (Band D / Wave 4)
...
- **Overlap with POS provider strategy:** Minimal — `013120_Boundary_Integration_Status_Projection.md` references integration status conceptually but is not a provider strategy document.
...
- **After dedupe:** Should contain **16 unique strategy files** + **new numbered README** (recommended `014000_Readme_POS_Provider_Integration_Strategy.md`).
...
- Archive moves are reversible if copies preserved under `docs/00999_conflicts/` per governance.
...
| --- | --- | --- |
| 1 | Archive 16 duplicate files from `13000_security` (5) and `14000` (11)? | **Yes** — preserve under `docs/00999_conflicts/duplicate_review/` or mark `archived` in index |
...
1. Move 16 duplicate files to `docs/00999_conflicts/duplicate_review/13000_14000_wave4b/` (or delete only after archive copy — **do not delete without archive** per governance rules).
2. Create `014000_Readme_POS_Provider_Integration_Strategy.md` in `14000`.

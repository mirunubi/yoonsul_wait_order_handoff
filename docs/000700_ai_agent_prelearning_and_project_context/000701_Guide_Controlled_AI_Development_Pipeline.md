...
>
> **0-B 인계 조건 (2026-08-13 확정)**: 0-A 재수행(`601700`)의 1단계 업무규칙 선언에서
> 0-B로 넘기는 제약이 확정되었다 — `601702` §1.18.
> **이 제약이 0-B 1단계 업무규칙에 재선언되지 않으면 착수 검증에서 반려한다.**
>
...
- **0-B** Staff identity / session
  ...
  - ⛔ **0-A로부터의 인계 조건 (`601702` §1.18, 2026-08-13)**: 0-B 설계는 다음을 충족해야 한다 — ① `Person` – `User`/Auth Identity – Staff Assignment 간 명시적 연결 정의 ② 하나의 `User`가 복수 Store/Scope를 가질 수 있을 것 ③ **JWT authentication subject를 특정 Store의 `staff.id`에 종속시키지 않을 것** ④ 커스텀 세션이 별도의 인증 세계로 동작하지 않을 것. **이 제약이 0-B 1단계 업무규칙에 재선언되지 않으면 착수 검증에서 반려한다.** 근거: `601211`(2026-07-18) — `staff_login()`/`auth_sessions`와 Supabase JWT/`current_actor_id()` 사이에 연결이 없어 `JWT.sub = staff.id` 전제가 성립하지 않는다.
...
Company / Owner / Tenant / HQ / Store / User identity / Customer identity / Staff identity / Session / Role / Permission / Membership / Menu seed / Dining table.
...
|---|---|---|---|---|---|---|
| Company | 있음 | 있음 | 없음 | 없음 | 미확인 | 부분재사용 |
| Store | 있음 | 있음 | 있음 | 불완전 | 실패 | 수정후재사용 |
...
- **JWT/session/caller context**: `current_actor_id()`의 JWT 'sub' 읽는 방식, 실제 JWT를 jwt.io 등으로 직접 디코딩해보기.
